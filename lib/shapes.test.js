...
describe("Shape class", () => {
  ...
  it("should throw an error when invalid hexadecimal text and shape color is entered", () => {
    const word = "BRU";
    const textColor = "#gghhpp";
    const shapeColor = "#kkffpp";
    const shape = () => new Shape(word, shapeColor, textColor);
    expect(shape).toThrowError("Invalid Shape and Text Color");
  });
  it("should throw an error when invalid hexadecimal text color is entered", () => {
    const word = "BRU";
    const textColor = "#gghhpp";
    const shapeColor = "#aabbdd";
    const shape = () => new Shape(word, shapeColor, textColor);
    expect(shape).toThrowError("Invalid Text Color");
  });
  it("should throw an error when invalid hexadecimal shape color is entered", () => {
    const word = "BRU";
    const textColor = "#aaccee";
    const shapeColor = "#kkLLMM";
    const shape = () => new Shape(word, shapeColor, textColor);
    expect(shape).toThrowError("Invalid Shape Color");
  });
  it("should not throw an error when valid hexadecimal color is entered", () => {
    const word = "BRU";
    const textColor = "#aaccee";
    const shapeColor = "#aa99ee";
    const shape = () => new Shape(word, shapeColor, textColor);
    expect(shape).not.toThrowError();
  });
});
